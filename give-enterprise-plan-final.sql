...
BEGIN
    ...
    -- Initialize or update usage tracking for each usage type
    -- Campaigns usage
    SELECT EXISTS(
        SELECT 1 FROM public.subscription_usage 
        WHERE user_id = user_uuid AND usage_type = 'campaigns'
    ) INTO usage_record_exists;
    
    IF usage_record_exists THEN
    ...
    ELSE
        INSERT INTO public.subscription_usage (
            user_id, subscription_id, usage_type, usage_count, reset_date, created_at, updated_at
        ) VALUES (
    ...
    SELECT EXISTS(
        SELECT 1 FROM public.subscription_usage 
        WHERE user_id = user_uuid AND usage_type = 'refreshes'
    ) INTO usage_record_exists;
    ...
    SELECT EXISTS(
        SELECT 1 FROM public.subscription_usage 
        WHERE user_id = user_uuid AND usage_type = 'ai_responses'
    ) INTO usage_record_exists;
